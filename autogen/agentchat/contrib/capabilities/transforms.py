...
class MessageHistoryLimiter:
    ...
    def apply_transform(self, messages: List[Dict]) -> List[Dict]:
        ...
        if len(messages) < self._max_messages:
            return messages
        # Mark: if it is tool, be sure to includes tool_call message
        # Fix the tool_call and tool message error
        last_k_messages = self._max_messages
        if messages[-self._max_messages]['role'] == 'tool':
            last_k_messages += 1

        return messages[-last_k_messages :]
